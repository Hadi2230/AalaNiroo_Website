      ...
      <header className={`fixed top-0 left-0 right-0 w-full z-50 transition-all duration-300 ${
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center py-4" style={{ direction: dir }}>
            ...
            <Link
              to="/"
              className={`flex items-center gap-4 ${dir === 'rtl' ? 'col-start-3 justify-self-end' : 'col-start-1 justify-self-start'}`}
            >
              {company.logoUrl ? (
            ...
            <div className="container mx-auto px-4 py-4">
              <nav className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-white/10">
                  <Button variant="outline" className="border-white/40 text-white w-full hover:bg-white/10 text-sm" onClick={() => openModal()}>
                    <Calendar className="w-3 h-3 mr-2 text-cyan-300" />
                    {language === 'fa' ? 'درخواست رزرو جلسه' : 'Book Meeting Request'}
                  </Button>
                  <Button className="bg-cyan-500 hover:bg-cyan-600 text-white w-full text-sm" onClick={showContactMessage}>
                    <Phone className="w-3 h-3 mr-2 text-cyan-300" />
                    {t('hero.cta.call')}
                  </Button>
                </div>
              </nav>
